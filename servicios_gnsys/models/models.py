...
class servicios_gnsys(models.Model):
    ...
    tipo = fields.Selection([('1','SERVICIO DE PCOUNTER'),('2','RENTA MENSUAL DE LICENCIA EMBEDED')],string="Tipo de servicio")
    ...
    servActivo = fields.Boolean(string="Activo",compute='expiracionServicios', default=True)
    ...
    def expiracionServicios(self):
        for record in self:
            if record.contrato:
                _logger.info("-------Logger de OSWALDO "+str(record.contrato.fechaDeFinDeContrato.name))
