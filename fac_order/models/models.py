               ...
               for m in ff:              
                          ...
                          for k in p:
                              ...
                              if m.nombreAnte=='Renta base + costo de página procesada BN o color':
                                 p=''
                                 if m.contrato.cliente.id==1108:
                                    p=' MODELO '+str(product_id.name)+' Período ' + str(self.year) +' de ' +str(dict(self._fields['month'].selection).get(self.month))
                                 if k.x_studio_color_bn=='B/N':
                                 ...
                                 if k.x_studio_color_bn=='Color':
                                    self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_excedente':'si','x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pcolor,'product_uom_qty':colorp,'price_unit':m.clickExcedenteColor,'name':'(82121500) PAGINAS IMPRESAS COLOR: '+str(colorp)+'  INCLUYE ('+str(m.bolsaColor)+') SERIE : '+k.name+p })
                                    self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_excedente':'si','x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pbn,'product_uom_qty':bnp,'price_unit':m.clickExcedenteBN,'name':'(82121500) PAGINAS IMPRESAS: '+str(bnp)+' NEGRO INCLUYE ('+str(m.bolsaBN)+')  SERIE:'+k.name +p})                                                     
                              ...
                              if m.nombreAnte=='Renta base con ML incluidas BN o color + ML. excedentes' or m.nombreAnte=='Renta base con páginas incluidas BN o color + pag. excedentes':
                                if str(k.x_studio_estado)!='Back-up':
                                     ...
                                     if k.x_studio_color_bn=='B/N':
                                        if m.bolsaBN<bnp:
                                           bnp=bnp-m.bolsaBN
                                           self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_excedente':'si','x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pbn,'product_uom_qty':bnp,'price_unit':m.clickExcedenteBN,'x_studio_bolsa':m.bolsaBN,'name':'(82121500) PAGINAS IMPRESAS NEGRO: '+str(bnp)+' INCLUYE ('+str(m.bolsaBN)+') SERIE: '+k.name +p})                                                     
                                        else:    
                                     ...
                                     if k.x_studio_color_bn=='Color':
                                        if m.bolsaBN<bnp:
                                           bnpt=bnp 
                                           bnp=bnp-m.bolsaBN
                                           self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pbn,'product_uom_qty':bnp,'price_unit':m.clickExcedenteBN,'x_studio_bolsa':m.bolsaBN,'x_studio_excedente':'si','name':'(82121500) PAGINAS IMPRESAS NEGRO: '+str(bnpt)+' INCLUYE ('+str(m.bolsaBN)+') SERIE:'+k.name +p})                                                    
                                        else:
                                           self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_excedente':'si','x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pbn,'product_uom_qty':0,'price_unit':m.clickExcedenteBN,'x_studio_bolsa':m.bolsaBN,'name':'(82121500) PAGINAS IMPRESAS NEGRO: '+str(abs(int(currentPA.contadorMono)-int(currentP.contadorMono)))+' INCLUYE ('+str(m.bolsaBN)+') SERIE:'+k.name+p })                                                     
