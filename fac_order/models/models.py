                              ...
                              if m.nombreAnte=='Renta base con ML incluidas BN o color + ML. excedentes' or m.nombreAnte=='Renta base con páginas incluidas BN o color + pag. excedentes':
                                if str(k.x_studio_estado)!='Back-up':
                                     ...
                                     if k.x_studio_color_bn=='Color':
                                        if m.bolsaBN<bnp:
                                           bnp=bnp-m.bolsaBN
                                           self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pbn,'product_uom_qty':bnp,'price_unit':m.clickExcedenteBN,'x_studio_bolsa':m.bolsaBN,'x_studio_excedente':'si','name':'(82121500)  Bolsa ('+str(m.bolsaBN)+') PAGINAS IMPRESAS NEGRO '+str(bnp)+' INCLUYE ('+str(m.bolsaBN)+') SERIE:'+k.name })                                                    
                                        else:
                                           self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pbn,'product_uom_qty':0,'price_unit':m.clickExcedenteBN,'x_studio_bolsa':m.bolsaBN,'name':'(82121500) PAGINAS IMPRESAS NEGRO '+str(bnp)+' INCLUYE ('+str(m.bolsaBN)+') SERIE:'+k.name })                                                     
                                        if m.bolsaColor<colorp:
                                           colorp=colorp-m.bolsaColor                                    
                                           self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pcolor,'product_uom_qty':colorp,'price_unit':m.clickExcedenteColor,'x_studio_bolsa':m.bolsaColor,'x_studio_excedente':'si','name':'(82121500) PAGINAS IMPRESAS COLOR '+str(colorp)+' Bolsa ('+str(m.bolsaColor)+') SERIE:'+k.name })
                                        else:
                                           self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pcolor,'product_uom_qty':0,'price_unit':m.clickExcedenteColor,'x_studio_bolsa':m.bolsaColor,'name':'(82121500)  PAGINAS IMPRESAS COLOR '+str(colorp)+' Bolsa ('+str(m.bolsaColor)+') SERIE:'+k.name })                                      
                                     self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':rentaE,'product_uom_qty':1,'price_unit':m.rentaMensual,'name':'(80161801)  RENTA EQUIPO ' +k.x_studio_locacion_recortada+' SERIE: '+k.name +' MODELO: '+ k.product_id.name+" Período "+perido,'discount':int(self.x_studio_descuento)})                                                                                                                                      
               for j in ff:                      
