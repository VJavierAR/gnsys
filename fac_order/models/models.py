                                ...
                                if str(k.x_studio_estado)!='Back-up':
                                     if k.x_studio_color_bn=='B/N':
                                        if m.bolsaBN<bnp:
                                           ...
                                           self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pbn,'product_uom_qty':abs(int(currentPA.contadorMono)-int(currentP.contadorMono)),'price_unit':m.clickExcedenteBN,'x_studio_bolsa':m.bolsaBN,'name':'(82121500) PAGINAS IMPRESAS NEGRO: 0 INCLUYE ('+str(m.bolsaBN)+') SERIE:'+k.name })                                                     
                                     if k.x_studio_color_bn=='Color':
                                        if m.bolsaBN<bnp:
                                           bnpt=bnp 
                                           bnp=bnp-m.bolsaBN
                                        ...
                                        if m.bolsaColor<colorp:
                                           ...
                                           self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':pcolor,'product_uom_qty':abs(int(currentPA.contadorColor)-int(currentP.contadorColor)),'price_unit':m.clickExcedenteColor,'x_studio_bolsa':m.bolsaColor,'name':'(82121500)  PAGINAS IMPRESAS COLOR: 0 INCLUYE ('+str(m.bolsaColor)+') SERIE:'+k.name })                                      
                                     self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':m.id,'x_studio_field_9nQhR':k.id,'product_id':rentaE,'product_uom_qty':1,'price_unit':m.rentaMensual,'name':'(80161801)  RENTA EQUIPO ' +k.x_studio_locacion_recortada+' SERIE: '+k.name +' MODELO: '+ k.product_id.name+" Período "+perido,'discount':int(self.x_studio_descuento)})                                                                                                                                      
               for j in ff:                      
