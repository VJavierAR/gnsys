            ...
            if self.x_studio_dividir_servicios==False and self.x_studio_dividir_servicios_1==False and len(self.order_line)<1:
               ...
               for s in self.x_studio_servicios:
                     ...
                     if s.nombreAnte=='SERVICIO DE TFS' or s.nombreAnte=='OPERADOR TFS' or s.nombreAnte=='TFS' or s.nombreAnte=='SERVICIO DE TFS ' :                        
                        self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':s.id,'product_id':11414 ,'product_uom_qty':1.0,'price_unit':s.rentaMensual})                                                                                                    
                     if s.nombreAnte=='SERVICIO DE MANTENIMIENTO':                        
                        self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':s.id,'product_id':11415 ,'product_uom_qty':1.0,'price_unit':s.rentaMensual})                                                                                                    
                     if s.nombreAnte=='SERVICIO DE ADMINISTRADOR KM NET MANAGER':                        
                        self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':s.id,'product_id':11416 ,'product_uom_qty':1.0,'price_unit':s.rentaMensual})                                                                                                    
                     if s.nombreAnte=='PAGINAS IMPRESAS EN BN':                        
                        self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':s.id,'product_id':11417 ,'product_uom_qty':int(s.cantidad),'price_unit':s.rentaMensual})                                                                                                    
                     if s.nombreAnte=='RENTA MENSUAL DE LICENCIA  7 EMBEDED' or s.nombreAnte=='RENTA MENSUAL DE LICENCIA  14 EMBEDED' or  s.nombreAnte=='RENTA MENSUAL DE LICENCIA  2 EMBEDED':                        
                        self.env['sale.order.line'].create({'order_id': sale.id,'x_studio_servicio':s.id,'product_id':11418 ,'product_uom_qty':1.0,'price_unit':s.rentaMensual})                                                                                                    
