...
class PartnerXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, ticket):
        ...
        sheet.merge_range('A1:S1', 'Tickets', merge_format)
        for obj in ticket:
            tipo=''
    ...
    def generate_xlsx_report(self, workbook, data, ruta):
        ...
        for obj in ruta:
            for orden in obj.ordenes:
                ...
                sheet.write(i, 14, orden.partner_id.street_name if(orden.partner_id.street_name) else '', bold)
                sheet.write(i, 15, orden.partner_id.street_number2 if(orden.partner_id.street_number2) else '', bold)
                sheet.write(i, 16, orden.partner_id.street_number if(orden.partner_id.street_number) else '', bold)
                sheet.write(i, 17, orden.partner_id.l10n_mx_edi_colony if(orden.partner_id.l10n_mx_edi_colony) else '', bold)
                sheet.write(i, 18, orden.partner_id.l10n_mx_edi_locality if(orden.partner_id.l10n_mx_edi_locality) else '', bold)
