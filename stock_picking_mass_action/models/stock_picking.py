...
class StockPicking(Model):
    ...
    def cambio_wizard(self):
        ...
        for p in self.move_ids_without_package:
            data={'almacen':p.location_id.x_studio_field_JoD2k.id,'move_id':p.id,'rel_cambio':wiz.id,'producto1':p.product_id.id,'producto2':p.product_id.id,'cantidad':p.product_uom_qty,'serie':p.x_studio_serie_destino.id,'tipo':self.picking_type_id.id}
