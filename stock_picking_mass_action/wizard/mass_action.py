    ...
    def confirmar(self):
        if(self.guia):
            ...
            if(self.pick.sale_id.x_studio_field_bxHgp):
                ...
                self.env['helpdesk.diagnostico'].sudo().create({ 'ticketRelacion' : self.pick.sale_id.x_studio_field_bxHgp.id, 'create_uid' : self.env.user.id, 'estadoTicket' : "Guia Agregada", 'comentario':self.guiaa}) 
