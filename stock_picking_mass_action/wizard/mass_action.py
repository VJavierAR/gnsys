...
class HelpdeskTicketMassAction(TransientModel):
    ...
    def report(self):
        ...
        d=self.env['helpdesk.ticket'].search(i,order='create_date asc').filtered(lambdax:x.x_studio_equipo_por_nmero_de_serie_1!=False or x.x_studio_equipo_por_nmero_de_serie!=False)
        if(len(d)>0):
            d[0].write({'x_studio_arreglo':str(d.mapped('id'))})
