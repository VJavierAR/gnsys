            ...
            if(mimetype=='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' or mimetype=='application/vnd.ms-excel'):
                ...
                for row_num, row in enumerate(sheet.get_rows()):
                    if(i>0):
                        ...
                        producto=self.env['product.product'].create({'default_code':row[1].value,'x_studio_field_ry7nQ':productid.id,'description':row[4].value,'name':row[4],'uom_id':unidad.id if(unidad.id) else False})
                        if(check):
                            if(almacen):
